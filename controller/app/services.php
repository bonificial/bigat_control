...
function retrieveServicesJSON($data)
{
    global $exec;
    //  $sql_where = (isset($data['category'])) ? ' WHERE post_category = "' . $data['category'] . '"' : '';
    $allServices = $exec->run_fetch_query("SELECT services.* FROM services");
    $response = ['success'=>true,'services'=>$allServices];
    return $response;
}
...
if (isset($obj['loadServices'])) {
    header('Content-Type: application/json');

     echo json_encode(retrieveServicesJSON($obj));
}
