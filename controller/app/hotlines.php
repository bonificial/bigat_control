...
function retrieveHotlinesJSON($data)
{
    global $exec;
    //  $sql_where = (isset($data['category'])) ? ' WHERE post_category = "' . $data['category'] . '"' : '';
    $allHotlines = $exec->run_fetch_query("SELECT hotlines.* FROM hotlines");
    $response = ['success'=>true,'hotlines'=>$allHotlines];
    return $response;
}
...
if (isset($obj['loadHotlines'])) {
    header('Content-Type: application/json');

     echo json_encode(retrieveHotlinesJSON($obj));
}
